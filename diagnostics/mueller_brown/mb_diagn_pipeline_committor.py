...
vanilla_committor_error = np.full(len(lag_times_list), fill_value=np.nan)
lagstop_committor_error =  np.full(len(lag_times_list), fill_value=np.nan)
vanilla_committor_error_errbar = np.full(len(lag_times_list), fill_value=np.nan)
lagstop_committor_error_errbar = np.full(len(lag_times_list), fill_value=np.nan)
...
for tau_idx, tau in enumerate(lag_times_list.copy().astype(int)):
    ...
    vanilla_committor_errors_over_simulations = np.full(n_simulations, fill_value=np.nan)
    lagstop_committor_errors_over_simulations = np.full(n_simulations, fill_value=np.nan)
    for simulation_idx in range(n_simulations):
        params_str = ("lag_time={}".format(tau) + " self.n_clusters={}".format(n_clusters) +
        ...
        while(tries < max_tries and (simulation_is_bad)):
            ...
            traj = mb_pb.simulate_simulation()
            ...
            exp_lagstop_msm.calculate_correlation_matrix(stopped_process=True,
                                                         stop_state_index=np.concatenate([mb_pb.A_labels[lagstop_msm_name],
                                                         mb_pb.B_labels[lagstop_msm_name]]))
            
            Tmat = exp_lagstop_msm.Tmat
            state_A = mb_pb.A_labels[lagstop_msm_name]
            state_B = mb_pb.B_labels[lagstop_msm_name]
        ...
        pntwise_error = mb_pb.get_committor_pntwise_error(msm_name=vanilla_msm_name, stopped_process=False)
        ...
        mb_pb.draw_exp_function_2D_plot(narrowed_exp_domain, pntwise_error, plot_name=params_str, label="Vanilla committor error")
        print("Vanilla committor error =", np.sum(pntwise_error))
        vanilla_committor_errors_over_simulations[simulation_idx] = np.sum(pntwise_error)

        pntwise_error = mb_pb.get_committor_pntwise_error(msm_name=lagstop_msm_name, stopped_process=True)
        narrowed_exp_domain, pntwise_error = narrow_down_exp_func_domain(mb_pb.boltzmann_prob_on_clusters,
                                                                         exp_lagstop_msm.cluster_centers, pntwise_error)
        mb_pb.draw_exp_function_2D_plot(narrowed_exp_domain, pntwise_error, plot_name=params_str, label="Lagstop committor error")
        print("Lagstop committor error =", np.sum(pntwise_error))
        lagstop_committor_errors_over_simulations[simulation_idx] = np.sum(pntwise_error)

    vanilla_success_mask = ~np.isnan(vanilla_committor_errors_over_simulations)
    lagstop_success_mask = ~np.isnan(lagstop_committor_errors_over_simulations)
    if(np.all(~vanilla_success_mask) or np.all(~lagstop_success_mask)):
        lag_times_list[tau_idx] = np.nan
        continue
    vanilla_committor_error[tau_idx] = np.mean(vanilla_committor_errors_over_simulations[vanilla_success_mask])
    lagstop_committor_error[tau_idx] = np.mean(lagstop_committor_errors_over_simulations[lagstop_success_mask])
    
    vanilla_committor_error_errbar[tau_idx] = get_measurements_error(
        vanilla_committor_errors_over_simulations[vanilla_success_mask])
    lagstop_committor_error_errbar[tau_idx] = get_measurements_error(
        lagstop_committor_errors_over_simulations[lagstop_success_mask])

success_mask = ~np.isnan(lag_times_list)

lag_times_list = lag_times_list[success_mask]
vanilla_committor_error = vanilla_committor_error[success_mask]
lagstop_committor_error = lagstop_committor_error[success_mask]
vanilla_committor_error_errbar = vanilla_committor_error_errbar[success_mask]
lagstop_committor_error_errbar = lagstop_committor_error_errbar[success_mask]

np.save("data/mb_committor_error_of_lag_time_ntraj_{}_trajlength_{}_seed_{}.npy".format(n_traj, traj_length, seed), [lag_times_list, vanilla_committor_error, lagstop_committor_error, vanilla_committor_error_errbar, lagstop_committor_error_errbar])
